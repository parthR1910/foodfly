...
class CompatibilityProfileController extends GetxController {
  late CompatibilityProfile compatibilityProfile;
  //compatibilityAnswers is the varible of compatibilityProfile
  late Map<String, CompatibilityAnswer> compatibilityAnswers = {};
  init() {
    compatibilityProfile = CompatibilityProfile.fromJson(jsonData);
    compatibilityAnswers =
        Map.from(compatibilityProfile.compatibilityAnswers ?? {});
    compatibilityProfile = CompatibilityProfile.fromJson(jsonData);

    update();
  }

  onToggleAnswerSelection(String key, String ans, bool isSelected) {
    if (isSelected) {
      if (!compatibilityAnswers[key]!.seekerAnswer!.value!.contains(ans)) {
        compatibilityAnswers[key]!.seekingAnswer!.value!.remove(ans);
        compatibilityAnswers[key]!.seekerAnswer!.value!.add(ans);
        update();
      }
    } else {
      compatibilityAnswers[key]!.seekerAnswer!.value!.remove(ans);
      compatibilityAnswers[key]!.seekingAnswer!.value!.add(ans);
      update();
    }
  }

  checkAndUpdateDataToServer() {
    compatibilityAnswers.forEach((key, ans) {
      final apiAnswers = compatibilityProfile.compatibilityAnswers?[key];
      final userAnswers = ans;
      //seekers answers
      final apiSeekersAnswersList = apiAnswers?.seekerAnswer?.value ?? [];
      final userSeekersAnswersList = userAnswers.seekerAnswer?.value ?? [];
      print('-----$key------');
      print('apiSeekersAnswersList');
      print(apiSeekersAnswersList);
      print('userSeekersAnswersList');
      print(userSeekersAnswersList);
      //seeking answers
      final apiSeekingsAnswersList = apiAnswers?.seekingAnswer?.value ?? [];
      final userSeekingsAnswersList = userAnswers.seekingAnswer?.value ?? [];
      print('apiSeekingsAnswersList');
      print(apiSeekingsAnswersList);
      print('userSeekingsAnswersList');
      print(userSeekingsAnswersList);
      print('-----------');

      final isSeekingAnsEquls = const ListEquality()
          .equals(apiSeekingsAnswersList, userSeekingsAnswersList);
      final isSeekersAnsEquls = const ListEquality()
          .equals(apiSeekersAnswersList, userSeekersAnswersList);
      print(isSeekingAnsEquls);
      print(isSeekersAnsEquls);
      if (!isSeekingAnsEquls && !isSeekersAnsEquls) {
        //update Data to api
      }
    });
  }
...
class CompatibilityProfile {
  ...
  factory CompatibilityProfile.fromJson(Map<String, dynamic> json) {
    return CompatibilityProfile(
      ...
      compatibilityAnswers: {
        for (var key in json['compatibilityAnswers'].keys)
          key as String: CompatibilityAnswer.fromJson(
